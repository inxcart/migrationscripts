...
function ps1_6_0_11_media_server()
{
    $newSettings = $prevSettings = file_get_contents(_PS_ROOT_DIR_.'/config/settings.inc.php');

    if (preg_match_all('/define\(\'_MEDIA_SERVER_([1-3])_\',\s*?\'(.*?)\'\s*?\)/ism', $newSettings, $matches)) {
    ...
    $newSettings = preg_replace('/define\(\'_MEDIA_SERVER_[1-3]_\',\s*?\'.*?\'\s*?\);/ism', '', $newSettings);
